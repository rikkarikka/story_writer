...
class model(nn.Module):
  def __init__(self,args):
    ...
    self.sm = nn.Softmax()
    self.linout = nn.Linear((args.hsz*2)+(args.esz*2),args.hsz,bias=False)
    self.ilinout = nn.Linear(args.hsz*2,args.hsz,bias=False)
    self.elinin = nn.Linear(args.hsz, args.hsz,bias=False)
    self.tanh = nn.Tanh()
  ...
  def validate(self,inp):
    ...
    def change_inter(previnter,intercatattn,hinter,cinter):
    ...
    prevn = Variable(torch.cuda.LongTensor(inp.size(0),1).fill_(3))
    prevv = Variable(torch.cuda.LongTensor(inp.size(0),1).fill_(3))
    prevnemb = self.internoun(prevn)
    prevvemb = self.interverb(prevv)
    previnter = torch.cat((prevnemb,prevvemb),2)
    catattn = Variable(torch.cuda.FloatTensor(inp.size(0),1,self.args.esz*2).zero_())
    info = []
    nouns = []
    verbs = []
    for i in range(6):
      nextinter, catattn, hinter, cinter, n, v = change_inter(previnter,catattn, hinter, cinter)
      _, nmax = torch.max(n,2)
      _, vmax = torch.max(v,2)
      prevn = nmax
      prevv = vmax
      nouns.append(nmax)
      verbs.append(vmax)
      ...
      prevvemb = self.interverb(prevv)
      previnter = torch.cat((prevnemb,prevvemb),2)
      info.append(previnter)
    info = torch.cat(info,1)
    print(info.size())
    outputs = []
    for i in range(self.args.maxlen):
      if i == 0:
        ...
        _,prev = torch.max(self.gen(dcatattn),2)
      ...
      #attend on enc
      q = self.elinin(decout.squeeze(1)).unsqueeze(2)
      w = torch.bmm(enc,q).squeeze(2)
      w = self.sm(w)
      ecc = torch.bmm(w.unsqueeze(1),enc)
      ...
      op = torch.cat((ecc,cc,decout),2)
      dcatattn = self.drop(self.tanh(self.linout(op)))
      outputs.append(self.gen(dcatattn))

    outputs = torch.cat(outputs,1)
    nouns = torch.cat(nouns,1)
    verbs = torch.cat(verbs,1)
    print(outputs.size(),nouns,verbs)
    return outputs, nouns, verbs
...
def valid(M,DS,args,jl):
  ...
  while True:
    ...
    loss = jl.vloss(targets,logits)
    loss = loss.data.cpu()[0]
    validloss.append(loss)
    print(loss)

    if len(validloss)%100==99: print(validloss[-1])
  M.train()
