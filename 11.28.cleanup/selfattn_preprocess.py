...
class load_data:
  ...
  def pad_batch(self,batch,targ=True):
    ...
    if targ:
      ...
      for x in targtmp:
        idxs = [k+1 for k,v in enumerate(x) if v==self.endtok]
        lastk = 0
        targspl.append([])
        for k in idxs[:-1]:
          targspl.append(x[lastk:k])
          lastk = k
        slens.append(len(idxs))
      m = max([len(x) for x in targtmp])
      targtmp = [x+([0]*(m-len(x))) for x in targtmp]
      targs = torch.cuda.LongTensor(targtmp)
      m = max([len(x) for x in targspl])
      targtmp = [x+([0]*(m-len(x))) for x in targspl]
      targspl = torch.cuda.LongTensor(targtmp)
    return (tensor,targs,targspl,slens)
