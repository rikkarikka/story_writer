...
from categorical_loss import CategoricalLoss
import numpy as np
...
def train(M,DS,args,optimizer):
  ...
  weights[0] = 0
  criterion = CategoricalLoss(DS.stoi_targets,args)#nn.CrossEntropyLoss(weights)
  trainloss = []
  for sources,targets,_ in data:
    sources = Variable(sources.cuda())
    targets = Variable(targets.cuda())
    M.zero_grad()
    outs,attns,_ = M(sources,targets)
    logits = M.gen(outs)
    ...
    loss = criterion(logits, targets,attns)
    loss.backward()
    trainloss.append(loss.data.cpu()[0])
    optimizer.step()
  return sum(trainloss)/len(trainloss)

def drawEncAttns(fn,sources,attns,vocab,svocab):
  data = [",".join([' ']+vocab)]
  attns = attns.data.cpu()
  for i,s in enumerate(sources):
    tmp = [' '.join(svocab[w] for w in s)]
    tmp += [str(x) for x in attns[i,:]]
    data.append(",".join(tmp))
  with open(fn,'w') as f:
    f.write("\n".join(data))
  
def validate(M,DS,args,vocab):
  M.eval()
  data = DS.val_batches
  #weights = torch.cuda.FloatTensor(args.vsz).fill_(1)
  #weights[0] = 0
  ...
  for sources,targets,swords in data:
    ...
    with open(args.savestr+args.epoch+".generated",'w') as f:
      f.write("\n".join(gentexts))
    drawEncAttns(args.savestr+args.epoch+".heatmap",swords,eW,vocab,DS.itos_sources)
    if args.debug:
      for x in gentexts:
        print(x)
...
def parseParams():
    ...
    parser.add_argument('-eventvocab', type=str, default="data/categories.txt")
