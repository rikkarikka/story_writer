...
class model(nn.Module):
  ...
  def beamsearch(self,inp):
    ...
    for i in range(self.beamsize):
      ...
      encbeam.append(torch.cat((enc,nemb,vemb),1))
    ...
    for i in range(self.args.maxlen):
      ...
      for j in range(len(beam)):
        ...
        if changej>0:
          ...
          encbeam[j] = torch.cat((ogenc, nemb, vemb),1)
        enc = encbeam[j]
        dembedding = self.decemb(prev[j].view(1,1))
        #print(dembedding.size(),ops[j].size(),interstack[j].size())
        decin = torch.cat((dembedding.squeeze(1),ops[j].squeeze(0)),1).unsqueeze(1)
  ...
  def forward(self,inp,verbs,nouns,out=None):
    ...
    nstack = torch.stack(([nemb[i,infostack[i],:] for i in range(inp.size(0))]),0).unsqueeze(1)
    vstack = torch.stack(([vemb[i,infostack[i],:] for i in range(inp.size(0))]),0).unsqueeze(1)
    enc = torch.cat((ogenc, nstack, vstack),1)
    #decode
    ...
    for i in range(outp): 
      if i == 0:
        ...
        for k in changes:
          ...
          if k-last>0:
            opstack.append(op[last:k])
          opstack.append(iout)
          last = k+1
          changed=True
        if changed:
          if last<op.size(0):
            opstack.append(op[last:])
          op = torch.cat(opstack,0)
